import pandas as pd
import numpy as np
from scipy.stats import jf_skew_t, ks_2samp
import sys
...
def ejecutar_simulacion(cantidad_o_archivo):
    # Parámetros jf_skew_t validados previamente
    params = {
        'a': 20.125100915646357, 'b': 5.031910403639798,
        'loc': -0.11329958736453954, 'scale': 5.830771215651215
    }
    ...
    try:
        ...
        simulados = jf_skew_t.ppf(u, **params)
        ...
        # Output
        print("\n" + "="*45)
        print(f"RESULTADOS DE LA CORRIDA ({len(simulados)} días)")
        print(f"P-Valor: {p_valor:.4f} | {'ACEPTADA' if p_valor > 0.05 else 'RECHAZADA'}")
        print("="*45)

        # Guardar todo
        pd.DataFrame({'u': u, 'demanda': simulados}).to_csv('simulacion_t_skew.csv', index=False)
        print("\n[OK] Datos guardados en 'simulacion_t_skew.csv'")

    except Exception as e:
        print(f"Error: {e}")

if __name__ == "__main__":
    entrada = sys.argv[1] if len(sys.argv) > 1 else '90' # 90 por defecto si no hay input
